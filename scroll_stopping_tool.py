...
class ScrollStoppingTool:
    ...
    def create_widgets(self):
        ...
        self.breaks_label.grid(row=0, column=2, sticky=tk.W)
    ...
    def open_settings(self):
        ...
        # Daily limit
        ttk.Label(settings_window, text="Daily Limit (minutes):").grid(row=0, column=0, padx=10, pady=10)
        limit_var = tk.StringVar(value=str(self.settings['daily_limit']))
        limit_entry = ttk.Entry(settings_window, textvariable=limit_var)
        limit_entry.grid(row=0, column=1, padx=10, pady=10)
        
        # Break reminder
        ttk.Label(settings_window, text="Break Reminder (minutes):").grid(row=1, column=0, padx=10, pady=10)
        reminder_var = tk.StringVar(value=str(self.settings['break_reminder']))
        reminder_entry = ttk.Entry(settings_window, textvariable=reminder_var)
        reminder_entry.grid(row=1, column=1, padx=10, pady=10)
        
        # Notifications
        notif_var = tk.BooleanVar(value=self.settings['notifications_enabled'])
        notif_check = ttk.Checkbutton(settings_window, text="Enable Notifications", 
                                     variable=notif_var)
        notif_check.grid(row=2, column=0, columnspan=2, padx=10, pady=10)
        
        # Auto break
        auto_break_var = tk.BooleanVar(value=self.settings['auto_break'])
        auto_break_check = ttk.Checkbutton(settings_window, text="Auto Break Reminders", 
